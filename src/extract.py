...
def longitude_to_srtm90_grid_long(longitude: float):
    ...
    """
    return int(1 + (180 + longitude - (longitude % 5.0)) / 5)
...
def latitude_to_srtm90_grid_lat(latitude: float):
    ...
    """
    return int((60 - (latitude - (latitude % 5.0))) / 5)
...
def get_country_bounds(country_gpkg=None):
    ...
    if country_gpkg is None:
        ...
        country_bnd = np.array([41.81458282, 12.10819435, 54.53541565, 18.99999809])
    else:
...
def determine_srtm_90_zip(country_gpkg=None):
    ...
    grid_long = list(
        range(
            longitude_to_srtm90_grid_long(min(tru_long)),
    ...
    grid_lat = list(
        range(
            latitude_to_srtm90_grid_lat(max(tru_lat)),
...
def determine_srtm_30_zip(country_gpkg=None):
    ...
    while glon <= max(tru_long):
        ...
        while glat <= max(tru_lat):
            gcoord = latitude_to_srtm30_text_lat(glat) + longitude_to_srtm30_text_long(
                glon
            )
...
def fetch_srtm_30_zips(
    destination_folder: str, country_gpkg=None, force_download=False
):
    """
    ...
    for zipf in determine_srtm_30_zip(country_gpkg=country_gpkg):
        ...
        if force_download or not os.path.exists(zip_local_uri):
            logger.info(f"Downloading {zip_web_uri} to {zip_local_uri}")
            if session is None:
            ...
            try:
                ...
                logger.info(f"Downloaded {zipf} to {zip_local_uri}")
            except HTTPError:
                logger.warning(
                    "Couldn't download %s from %s (%s error)"
...
def fetch_srtm_90_zips(
    ...
    for zipf in determine_srtm_90_zip(country_gpkg=country_gpkg):
        ...
        # TODO: skip file if (web hosted) zip file not found
        if force_download or not os.path.exists(zip_local_uri):
            requests_api.download_url(
                zip_web_uri, zip_local_uri, chunk_size=1024 * 1024
            )
        zip_list_out.append(zip_local_uri)
...
def unzip_raster_to_folder(zip_list: list, out_folder: str, ext=".tif"):
    ...
    if not ext.startswith("."):
        logger.warning(f"Amending extension to '.{ext}'")
        ext = "." + ext
...
def mosaic_rasters(input_raster_uri_list, output_geotiff_uri, nullvalue=-9999):
    ...
    with rasterio.open(input_raster_uri_list[0], "r") as inraster:
        ...
        if inraster.res[0] != inraster.res[1]:
            ...
            logger.info(
                "Pixel resolution of %d units (assumed metres)" % inraster.res[0]
            )
    ...
    if "crs" not in mainmeta.keys():
        raise NotImplementedError(
            f"Cannot deal with lack of CRS in {input_raster_uri_list[0]}"
        )
    ...
    for one_raster_uri in input_raster_uri_list:
        with rasterio.open(one_raster_uri, "r") as inraster:
            ...
            if inraster.meta["crs"] != mainmeta["crs"]:
                err_msg = f"Cannot deal with lack of CRS in {one_raster_uri}"
    ...
    fullbound = tuple(
        [
            abs(allbound["top"] - allbound["bottom"]),
    ...
    newdims = [
        np.int(np.ceil(fullbound[0] / mainres[1])),
...
def transform_raster_to_metre_projection(
    country_geotiff_uri: str, destination_epsg: str, reprojected_geotiff_uri: str
):
    """
    ...
    with rasterio.open(country_geotiff_uri) as src:
        dst_transform, dst_width, dst_height = warp.calculate_default_transform(
            src.crs, destination_epsg, src.width, src.height, *src.bounds
        )
        src_data = src.read(1)
        src_meta = src.meta
        src_crs = src.crs
...
def get_srtm30_for_country(
    ...
    srtm30_zip_list = fetch_srtm_30_zips(
        zip_download_folder, country_gpkg=country_gpkg, force_download=force_download
    )
    srtm30_hgt_list = unzip_raster_to_folder(
        srtm30_zip_list, download_folder, ext=".hgt"
    )
    ...
    transform_raster_to_metre_projection(
        temp_mosaic_uri, destination_epsg, output_geotiff_uri
    )
    os.remove(temp_mosaic_uri)
    return output_geotiff_uri


def get_srtm90_for_country(
    output_geotiff_uri: str,
    download_folder: str,
    destination_epsg: str,
    country_gpkg=None,
    ...
    zip_download_folder = download_folder + os.sep + "zip"
    srtm90_zip_list = fetch_srtm_90_zips(
        zip_download_folder, country_gpkg=country_gpkg, force_download=force_download
    )
